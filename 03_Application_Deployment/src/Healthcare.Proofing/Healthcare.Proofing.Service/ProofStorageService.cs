...
namespace Healthcare.Proofing.Service
{
    public class ProofStorage
    {
        ...
        //async private Task<ContractTransaction> UpdateProofDocument(string bindingId, DocProof storedDocProof, string description)
        //{
        //    var contractTransaction = await GetRecentTransactionByBindingId(bindingId);

        //    Profile _profile = contractTransaction.BusinessContractDTO;
        //    _profile.IdentifyProofs = storedDocProof;
        //    _profile.Status = ProfileStatus._1;

        //    return await _appProxy.UpdateProfileAsync(_profile, bindingId, description);

        //}

        //async private Task<ContractTransaction> GetRecentTransactionByBindingId(string bindingId)
        //{
        //    ICollection<ContractTransaction> contractTransactions = await _appProxy.GetTransactionsByBindingIdAsync(bindingId);
        //    ContractTransaction latestContract = contractTransactions.OrderBy(t => t.TransactionTime ?? DateTimeOffset.MaxValue).Last();

        //    //ContractTransaction transaction = await _trackerProxy.
